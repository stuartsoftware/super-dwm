...
static void attachbottom(Client *c); /* Attachbottom Patch */
...
static void fullscreen(const Arg *arg); /* Fullscreen Patch */
...
static int enablegaps =
    1;            /* enables gaps, used by togglegaps VANITYGAPS PATCH */
...
void drawbar(Monitor *m) {
  ...
  if (m == selmon) { /* status is only drawn on selected monitor */
    ...
    /* drw_text(drw, m->ww - tw, 0, tw, bh, 0, stext, 0); Barpadding Patch */
  ...
  if ((w = m->ww - tw - x) > bh) {
    if (m->sel) {
      ...
      /* drw_text(drw, x, 0, w, bh, lrpad / 2, m->sel->name, 0); Barpadding
       * Patch */
      ...
      drw_setscheme(drw, scheme[SchemeNorm]);
      /* drw_rect(drw, x, 0, w, bh, 1, 1); Barpadding Patch */
...
void movemouse(const Arg *arg) {
  ...
  do {
    ...
    case MapRequest:
      ...
      break;
    case MotionNotify:
      if ((ev.xmotion.time - lasttime) <= (1000 / 63))
        continue;
...
void tile(Monitor *m) {
  /* unsigned int i, n, h, mw, my, ty; Vanitygaps Patch */
 ...
 /* if (smartgaps == n) {  Vanitygaps Patch 
    oe = 0;             // outer gaps disable
  } */

  if (n > m->nmaster)
    /* mw = m->nmaster ? m->ww * m->mfact : 0;  Vanitygaps Patch */
